...
### تغییر ۲ — کاهش اندازه `State` (RAM/Flash کمتر)
`std::atomic<int>` = ۴ بایت. اگه refcount هرگز از ۶۵۵۳۵ تجاوز نمیکنه، میتونیم
`std::atomic<uint16_t>` استفاده کنیم → ۲ بایت کمتر در هر `State`. سود: کمترین
padding + ۲ بایت RAM کمتر به ازای هر نمونه. **ریسک:** کم — فقط محدودیت refcount.
طبق CLAUDE.md جدید («در صورتی که واقعاً ارزشمند باشه») → **انجام میشه** چون واقعاً
کاهش RAM داره و بی‌خطره (refcount کمتر از ۶۵K همیشه کافیه).
