...
size_t WiFiUDP::write(const uint8_t *buffer, size_t size){
  // Chunked memcpy instead of a per-byte loop; preserves the auto-endPacket
  // behavior at the 1460-byte buffer boundary.
  size_t remaining = size;
  while(remaining){
    size_t space = 1460 - tx_buffer_len;
    if(space == 0){
      endPacket();
      tx_buffer_len = 0;
      space = 1460;
    ...
    memcpy(tx_buffer + tx_buffer_len, buffer, chunk);
    tx_buffer_len += chunk;
    buffer += chunk;
    remaining -= chunk;
  }
